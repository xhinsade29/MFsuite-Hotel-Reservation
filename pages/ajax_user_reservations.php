...
if ($res && $res->num_rows > 0):
    while ($booking = $res->fetch_assoc()):
        ...
        if (!empty($room_type_id) && is_numeric($room_type_id)) {
            ...
            $service_result = $mycon->query($service_sql);
        ...
        if (count($service_names) > 0) {
            $service_names_escaped = array_map(function($name) use ($mycon) {
                return "'" . $mycon->real_escape_string($name) . "'";
            }, $service_names);
            ...
            $service_price_result = $mycon->query($service_price_sql);
        ...
        if (!empty($booking['assigned_room_id'])) {
            ...
            $room_num_result = $mycon->query($room_num_sql);
