...
if ($active_res && mysqli_num_rows($active_res) > 0) {
    while ($row = mysqli_fetch_assoc($active_res)) {
        ...
        if ($status === 'approved') {
            ...
            echo '<button class="btn btn-primary btn-sm mark-complete-btn" data-reservation-id="' . $row['reservation_id'] . '" data-checkout-time="' . htmlspecialchars($checkout_time) . '">Mark as Completed</button>';
        } else if ($status === 'completed') {
            echo '<button class="btn btn-success btn-sm" disabled>Already Completed</button>';
        }
        echo '</td>';
        echo '</tr>';
    }
